<footer class="bg-gray-100 border-t pt-6 md:pt-8 pb-4 text-sm text-gray-700">
    <div class="container mx-auto px-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
        <div class="mb-4 md:mb-0">
            <h4 class="font-bold mb-3 text-base md:text-sm" style="color: #ed1c24;">{{ config('texts.footer_policy_title') }}</h4>
            <ul class="space-y-0.5">
                @if(isset($policies) && $policies->count() > 0)
                @foreach($policies as $index => $policy)
                <li><a href="{{ route('page.detail', ['alias' => $policy->link]) }}" class="font-bold text-gray-900 hover:text-red-600 text-sm block py-0.5">{{ $policy->name }}</a></li>
                @endforeach
                @endif
            </ul>
        </div>
        <div class="mb-4 md:mb-0">
            <h4 class="font-bold mb-3 text-base md:text-sm" style="color: #ed1c24;">{{ config('texts.footer_legal_title') }}</h4>
            <div class="font-bold text-gray-900 text-sm leading-relaxed">
                {!! $settings->legal_regulations !!}
            </div>
        </div>
        <div class="mb-4 md:mb-0">
            <h4 class="font-bold mb-3 text-base md:text-sm" style="color: #ed1c24;">{{ config('texts.footer_connect_title') }}</h4>
