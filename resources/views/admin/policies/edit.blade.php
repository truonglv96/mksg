...
$breadcrumbs = [
    ...
    ['label' => 'Chỉnh sửa', 'url' => route('admin.policies.edit', $policy->id)]
];
@endphp
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6 fade-in">
    <div class="flex items-center justify-between mb-6 pb-4 border-b border-gray-200">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Cập nhật nội dung và cấu hình hiển thị</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="content"
                                      ...
                                      placeholder="Nhập nội dung chính sách...">{{ old('content', $policy->content) }}</textarea>
                            @error('content')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
            ...
            <div class="space-y-6">
                <div class="bg-gray-50 rounded-lg p-6 border border-gray-200">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="number"
                                   name="weight"
                                   value="{{ old('weight', $policy->weight ?? 0) }}"
                                   min="0"
                        ...
                        <div class="flex items-center gap-2">
                            <input type="checkbox"
                                   ...
                                   {{ old('status', $policy->status == \App\Models\Page::IS_ACTIVE) ? 'checked' : '' }}>
                ...
                <div class="bg-gray-50 rounded-lg p-6 border border-gray-200">
                    ...
                    @if($policy->image)
                        <div class="mb-4">
                            <img src="{{ $policy->getImage() }}" alt="{{ $policy->name }}" class="w-full rounded-lg border border-gray-200">
                        </div>
                        <div class="flex items-center gap-2 mb-4">
                            ...
                            <label for="delete_image" class="text-sm text-gray-700">Xóa ảnh hiện tại</label>
                        </div>
                    @endif
                    <input type="file"
                           name="image"
...
<script>
    (function() {
        ...
        linkInput.addEventListener('input', function() {
            linkTouched = linkInput.value.trim().length > 0 && linkInput.value.trim() !== initialLink;
        });
        ...
        nameInput.addEventListener('input', function() {
            ...
            if (initialLink === '' || linkInput.value.trim() === initialLink) {
