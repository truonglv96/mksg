                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="type_sale" 
                                ...
                                <option value="0" {{ old('type_sale', $product->type_sale ?? 0) == 0 ? 'selected' : '' }}>Tại Shop & Online</option>
                                <option value="1" {{ old('type_sale', $product->type_sale ?? 0) == 1 ? 'selected' : '' }}>Tại Shop</option>
                                <option value="2" {{ old('type_sale', $product->type_sale ?? 0) == 2 ? 'selected' : '' }}>Online</option>
                            </select>
                            @error('type_sale')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
...
function addSalePriceRow(category1 = '', category2 = '', price = '', index = null) {
    ...
    // Get categories HTML from first select or build from allCategories
    ...
    if (category1 && typeof allCategories !== 'undefined') {
        const childCategories = allCategories.filter(cat => cat.parent_id == category1);
        childCategories.forEach(cat => {
            const selected = cat.id == category2 ? 'selected' : '';
            category2Options += `<option value="${cat.id}" ${selected}>${cat.name}</option>`;
        });
    }
